...
class TestVideoProcessorEdgeCases:
    ...
    def test_corrupted_video_file(self, analyzer, tmp_path):
        ...
        # Create partially written file
        with open(corrupted_path, 'wb') as f:
            f.write(b'corrupted data')
        
        # Changed from ValueError to VideoProcessingError
    ...
    def test_video_with_zero_frames(self, analyzer, tmp_path):
        ...
        out.release()  # Release without writing frames
        
        # This will raise VideoProcessingError because OpenCV can't open it
        with pytest.raises(VideoProcessingError):
            output_path = tmp_path / "output.mp4"
    ...
    def test_invalid_output_path(self, analyzer, tmp_path):
        ...
        # Platform-specific readonly paths
        if platform.system() == 'Windows':
            # On Windows, try to write to system directory
            readonly_path = "C:\\Windows\\System32\\output.mp4"
        else:
            # On Unix-like systems
            readonly_path = "/root/output.mp4"
        
        # Expect VideoProcessingError which wraps permission errors
